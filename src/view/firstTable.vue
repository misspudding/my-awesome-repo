...
export default {
  ...
  computed: {
    ...mapState('moduleA/selectTableData', ['originalList','originalSelectedColumns']),
  ...
  methods:{
    ...mapActions('moduleA/selectTableData', ['fetchList', 'setSelectedColumns']),
...
getSelectedColumnsProp(){
    ...
    saveConfig() {
      const list = this.modules[this.selectedModule][this.selectedSituation]
      this.tableColumnsCopy = this.selectedColumnsProp.map(itemB => 
      list.find(itemA => itemA.prop === itemB)).filter(item => item !== undefined)
